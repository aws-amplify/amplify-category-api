...
const PROJECT_NAME = 'interactiontest';
...
const spawn = (command: string | string[], params: string[] = [], options: SpawnOptions = {}) => {
...
const getCLIPath = (testingWithLatestCodebase = false) => {
...
const initJSProjectWithProfile = (cwd: string, settings?: Partial<typeof defaultSettings>): Promise<void> => {
  ...
  const chain = spawn(getCLIPath(), cliArgs, { cwd, env, disableCIDetection: s.disableCIDetection })
...
const addApiWithoutSchema = (cwd: string, opts: Partial<AddApiOptions & { apiKeyExpirationDays: number }> = {}) => {
  const options = _.assign(defaultAddApi, opts);
  return spawn(getCLIPath(options.testingWithLatestCodebase), ['add', 'api'], { cwd })
...
function updateSchema(projectDir: string, projectName: string, schemaText: string) {
...
function updateApiSchema(cwd: string, projectName: string, schemaText: string, forceUpdate: boolean = false) {
  if (forceUpdate) {
    schemaText += '  ';
  }
...
const deleteProject = async (cwd: string, profileConfig?: any, usingLatestCodebase = false): Promise<void> => {
  return spawn(getCLIPath(usingLatestCodebase), ['delete'], { cwd, noOutputTimeout: pushTimeoutMS })
...
const runCodegen = async (cwd: string, usingLatestCodebase = false): Promise<void> => {
  return spawn(getCLIPath(usingLatestCodebase), ['codegen', 'add'], { cwd, noOutputTimeout: pushTimeoutMS })
...
function amplifyPush(cwd: string, testingWithLatestCodebase = false): Promise<void> {
  return spawn(getCLIPath(testingWithLatestCodebase), ['push'], { cwd, noOutputTimeout: pushTimeoutMS })
    .wait('Are you sure you want to continue?')
    .sendConfirmYes()
    .wait('Do you want to generate code for your newly created GraphQL API')
    .sendConfirmNo()
