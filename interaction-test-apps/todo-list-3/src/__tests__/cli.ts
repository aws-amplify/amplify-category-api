...
export const initJSProjectWithProfile = (cwd: string, settings?: Partial<typeof defaultSettings>): Promise<void> => {
...
export const addApiWithoutSchema = (cwd: string, opts: Partial<AddApiOptions & { apiKeyExpirationDays: number }> = {}) => {
...
export function updateApiSchema(cwd: string, projectName: string, schemaText: string, forceUpdate: boolean = false) {
  if (forceUpdate) {
    schemaText += '  ';
  }
...
export const runCodegen = async (cwd: string, usingLatestCodebase = false): Promise<void> => {
