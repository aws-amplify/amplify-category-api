...
describe('Migration does not inherit auth', () => {
  ...
  test('No auth disallows access to imported table', async () => {
    const {
    ...
    expect(gen2Result.statusCode).toEqual(400);
    ...
    expect(gen1ListResult.body.data.listTodos.items.length).toEqual(1);
    expect(gen1ListResult.body.data.listTodos.items[0].id).toEqual(gen1Todo.id);
    ...
    expect(gen2ListResult.statusCode).toEqual(400);
    ...
    expect(listResult.statusCode).toEqual(400);
  });
});
