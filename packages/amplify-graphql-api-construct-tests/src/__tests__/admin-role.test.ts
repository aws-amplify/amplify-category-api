...
describe('CDK Auth Modes', () => {
  ...
  test('Can be invoked with Admin Roles defined', async () => {
    ...
    const name = await initCDKProject(projRoot, templatePath, {
      cdkVersion: '2.177.0', // Explicitly declaring this, since this version needs to match cognito idp
      additionalDependencies: [
        'esbuild', // required to bundle the lambda function
        '@aws-cdk/aws-cognito-identitypool-alpha@2.177.0-alpha.0', // using alpha cognito idp resource for auth config
