...
describe('searchable deployment when previous deployed state had node to node encryption enabled', () => {
  let projRoot: string;
  ...
  it('if previous deployment had NodeToNodeEncryption, carry it forward, there should be no data loss', async () => {
    ...
    expect(searchDomainPropsSecondDeploy.NodeToNodeEncryptionOptions.Enabled).toEqual(true);
  });
