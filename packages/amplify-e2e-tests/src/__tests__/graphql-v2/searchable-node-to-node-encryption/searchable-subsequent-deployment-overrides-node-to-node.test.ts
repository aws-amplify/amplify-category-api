...
describe('searchable deployment user flag overrides previous deployment state', () => {
  let projRoot: string;
  ...
  it('if previous deployment had no NodeToNodeEncryption, and flag is enabled on subsequent push, deploy, there should be no data loss', async () => {
    ...
    expect(searchDomainPropsFirstDeploy).not.toHaveProperty('NodeToNodeEncryptionOptions');
