...
export const testRdsIAMFieldAuth = (engine: ImportedRDSType, queries: string[]): void => {
  describe('RDS IAM field auth', () => {
    ...
    const initProjectAndImportSchema = async (): Promise<void> => {
      ...
      const schema = /* GraphQL */ `
        ...
        # IAM Public can't delete a record
        # IAM Public can't update SSN field
        # IAM Public can't read SSN field 
        # IAM Public can create a record with all fields, but can't read SSN response
        # IAM Public subscription can't read SSN field
        type Person1 @model @auth(rules: [{allow: private, provider: iam}, {allow: public, provider: iam}]) {
          id: Int! @primaryKey
          firstName: String
          lastName: String
          ssn: String @auth(rules: [{allow: private, provider: iam}, {allow: public, provider: iam, operations: [create]}])
        }

        # IAM Public can't create a record with SSN value
        # IAM Public can create a record without a SSN value
        # IAM Public can't update a record with SSN value
        # IAM Public can't delete a record
        type Person2 @model @auth(rules: [{allow: private, provider: iam}, {allow: public, provider: iam}]) {
          id: Int! @primaryKey
          firstName: String
          lastName: String
          ssn: String @auth(rules: [{allow: private, provider: iam}, {allow: public, provider: iam, operations: [read]}])
        }

        # IAM Public can't create a record with SSN value
        # IAM Public can delete a record
        type Person3 @model @auth(rules: [{allow: private, provider: iam}, {allow: public, provider: iam}]) {
          id: Int! @primaryKey
          firstName: String
          lastName: String
    ...
    test('Model Person1 - IAM Public can create a record with all fields, but SSN response is null', async () => {
      const person = {
        id: 1,
        firstName: 'Person1',
        lastName: 'Person1',
      ...
      // IAM Public can create the record with all fields. But the API throws unauthorized error for the SSN field.
      // We will run a get query to verify that the record was created with all the fields including SSN.
      await expect(person1IAMPublicClient.create('createPerson1', person)).rejects.toThrow(
    ...
    test('Model Person1 - IAM Public cannot read SSN field on GET query', async () => {
      const person = {
        id: 1,
        firstName: 'Person1',
        lastName: 'Person1',
      };
      await expect(person1IAMPublicClient.get({ id: person.id })).rejects.toThrow(
        'GraphQL error: Not Authorized to access ssn on type Person1',
      );

      const person1Response = await person1IAMPrivateClient.get({ id: person.id }, getQueryWithoutSSN('Person1'));
      expect(person1Response.data.getPerson1.id).toEqual(person.id);
      expect(person1Response.data.getPerson1.firstName).toEqual(person.firstName);
      expect(person1Response.data.getPerson1.lastName).toEqual(person.lastName);
    });

    test('Model Person1 - IAM Public cannot read SSN field on LIST query', async () => {
      const person = {
        id: 1,
        firstName: 'Person1',
        lastName: 'Person1',
      };
      await expect(person1IAMPublicClient.list()).rejects.toThrow('GraphQL error: Not Authorized to access ssn on type Person1');

      const listPerson1Response = await person1IAMPrivateClient.list(undefined, listQueryWithoutSSN('Person1'));
    ...
    test('Model Person1 - IAM Public cannot update SSN', async () => {
      ...
      await expect(person1IAMPublicClient.update('updatePerson1', person)).rejects.toThrow(
    ...
    test('Model Person1 - IAM Public can update without SSN', async () => {
      const person = {
        id: 1,
      ...
      const person1UpdateResponse = await person1IAMPublicClient.update('updatePerson1', person, selectionSetWithoutSSN);
    ...
    test('Model Person1 - IAM Public cannot delete a record', async () => {
      const person = {
        id: 1,
      };
      await expect(person1IAMPublicClient.delete('deletePerson1', person)).rejects.toThrow(
    ...
    test('Model Person1 - SSN field must be redacted on subscriptions', async () => {
      ...
      const subscriptionPromise = new Promise((resolve, _) => {
        subscription = observer.subscribe(
          ...
          (err) => {
            ...
            throw new Error('IAM Public client should be able to subscribe on Person1');
          },
        );
      });
      ...
      await person1IAMPrivateClient.create('createPerson1', personRecord);
    ...
    test('Model Person2 - IAM Public cannot create a record with SSN value', async () => {
      ...
      await expect(person2IAMPublicClient.create('createPerson2', person)).rejects.toThrow(
        'GraphQL error: Not Authorized to access createPerson2 on type Mutation',
      );

      const personResponse = await person2IAMPrivateClient.get({ id: person.id });
      expect(personResponse.data.getPerson2).toBeNull();
    });

    test('Model Person2 - IAM Public can create a record without SSN value', async () => {
      const person = {
        id: 1,
        firstName: 'Person1',
        lastName: 'Person1',
      };
      const personCreateResponse = await person2IAMPublicClient.create('createPerson2', person, selectionSetWithoutSSN);
    ...
    test('Model Person2 - IAM Public cannot update a record including SSN', async () => {
      const person = {
        id: 1,
        firstName: 'Person1',
        lastName: 'Person1',
      ...
      await expect(person2IAMPublicClient.update('updatePerson2', person)).rejects.toThrow(
        'GraphQL error: Not Authorized to access updatePerson2 on type Mutation',
      );
    });

    test('Model Person2 - IAM Public cannot delete a record', async () => {
      const person = {
        id: 1,
      };
      await expect(person2IAMPublicClient.delete('deletePerson2', person)).rejects.toThrow(
    ...
    test('Model Person2 - SSN field must be readable on subscriptions', async () => {
      ...
      const subscriptionPromise = new Promise((resolve, _) => {
        subscription = observer.subscribe(
          ...
          (err) => {
            console.log(JSON.stringify(err.error.errors, null, 4));
            throw new Error('IAM Public client should be able to subscribe on Person2');
          },
        );
      });
      ...
      await person2IAMPrivateClient.create('createPerson2', personRecord);
    ...
    test('Model Person3 - IAM Public cannot create a record with SSN value', async () => {
      const person = {
        id: 1,
        firstName: 'Person1',
        lastName: 'Person1',
      ...
      await expect(person3IAMPublicClient.create('createPerson3', person)).rejects.toThrow(
        'GraphQL error: Not Authorized to access createPerson3 on type Mutation',
      );

      const personResponse = await person3IAMPrivateClient.get({ id: person.id });
      expect(personResponse.data.getPerson3).toBeNull();
    });

    test('Model Person3 - IAM Public can delete a record', async () => {
      const person = {
        id: 1,
        firstName: 'Person1',
        lastName: 'Person1',
      ...
      const personCreateResponse = await person3IAMPrivateClient.create('createPerson3', person);
      ...
      // IAM Public can delete a record. It will throw unauthorized error for SSN field. The record will still be deleted.
      await expect(person3IAMPublicClient.delete('deletePerson3', { id: person.id })).rejects.toThrow(
        'GraphQL error: Not Authorized to access ssn on type Person3',
      );

      const person3GetResponse = await person3IAMPrivateClient.get({ id: person.id });
    ...
    test('Model Person3 - SSN field must be redacted on subscriptions', async () => {
      ...
      const subscriptionPromise = new Promise((resolve, _) => {
        subscription = observer.subscribe(
          ...
          (err) => {
            console.log(JSON.stringify(err.error.errors, null, 4));
            throw new Error('IAM Public client should be able to subscribe on Person3');
          },
        );
      });
      ...
      await person3IAMPrivateClient.create('createPerson3', personRecord);
