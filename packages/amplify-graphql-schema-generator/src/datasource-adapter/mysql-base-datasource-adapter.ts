...
export class MySQLSchema {
  ...
  constructor({ fields, indexes, tables }: { fields: string; indexes: string; tables: string }) {
  ...
  private setTables(tables: any): void {
  ...
  private async getPrimaryKey(tableName: string): Promise<Index | null> {
  ...
  private mapDataType(datatype: string, nullable: boolean, tableName: string, fieldName: string, columntype: string): FieldType {
  ...
  protected getEnumValues(value: string): string[] {
  ...
  protected generateEnumName(tableName: string, fieldName: string) {
