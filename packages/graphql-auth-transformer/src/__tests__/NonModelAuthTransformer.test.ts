...
describe('@auth directive without @model', () => {
  ...
  test('that @auth with no @model on type is failing validation as model is required', () => {
    ...
    expect(t).toThrow(`Types annotated with @auth must also be annotated with @model.`);
  });
  ...
  test('that @auth with no @model on field is failing validation when operations specified', () => {
    ...
    expect(t)
      .toThrow(`@auth rules on fields within types that does not have @model directive cannot specify 'operations' argument as there are \
operations will be generated by the CLI.`);
  ...
  test('that @auth on Query field is failing validation when operations specified', () => {
    ...
    expect(t).toThrow(`@auth rules on fields within Query, Mutation, Subscription cannot specify 'operations' argument as these rules \
