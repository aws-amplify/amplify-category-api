...
Static group authorization should perform as expected.`,
  ...
  private protectField(
    ...
    if (staticGroupRules && staticGroupRules.length) {
      ...
      templateParts.push(vtlPrinter.print(compoundExpression(expressions)));
    }
...
Either make the field optional, set auth on the object and not the field, or disable subscriptions for the object (setting level to off or public)\n`);
        ...
        resolver.Properties.ResponseMappingTemplate = vtlPrinter.print(
          this.resources.operationCheckExpression(ctx.getMutationTypeName(), field),
        );
      }
      // If a resolver exists, a @connection for example. Prepend it to the req.
      const templateParts = [vtlPrinter.print(authExpression), resolver.Properties.RequestMappingTemplate];
  ...
  private protectCreateForField(
    ...
    if (rules && rules.length && createResolverResource) {
      ...
      if (
        ...
        const templateParts = [
          vtlPrinter.print(iff(raw(`$ctx.args.input.containsKey("${field.name.value}")`), compoundExpression(expressions))),
...
operations will be generated by the CLI.`,
  ...
  private protectGetQuery(
    ...
    if (!rules || rules.length === 0 || !resolver) {
      ...
      if (authExpression) {
        const templateParts = [vtlPrinter.print(authExpression), resolver.Properties.ResponseMappingTemplate];
  ...
  private protectCreateMutation(
    ...
    if (!rules || rules.length === 0 || !resolver) {
      ...
      if (
        ...
        const templateParts = [vtlPrinter.print(compoundExpression(expressions)), resolver.Properties.RequestMappingTemplate];
        resolver.Properties.RequestMappingTemplate = templateParts.join('\n\n');
        ctx.setResource(resolverResourceId, resolver);
      }
    }
  ...
  private protectUpdateOrDeleteMutation(
    ...
    if (!rules || rules.length === 0 || !resolver) {
      ...
      if (
        ...
        const templateParts = [
          vtlPrinter.print(field && ifCondition ? iff(ifCondition, compoundExpression(expressions)) : compoundExpression(expressions)),
  ...
  private protectSearchQuery(ctx: TransformerContext, def: ObjectTypeDefinitionNode, resolverResourceId: string, rules: AuthRule[]) {
    ...
    if (!rules || rules.length === 0 || !resolver) {
      ...
      if (authExpression) {
        const templateParts = [
          vtlPrinter.print(this.resources.makeESItemsExpression(ctx.isProjectUsingDataStore())),
          vtlPrinter.print(authExpression),
          vtlPrinter.print(this.resources.makeESToGQLExpression()),
        ];
        resolver.Properties.ResponseMappingTemplate = templateParts.join('\n\n');
        ctx.setResource(resolverResourceId, resolver);
      }
  ...
  private addSubscriptionResolvers(
    ...
    if (!rules || rules.length === 0) {
      ...
      if ((staticGroupAuthorizationRules.length > 0 || ownerAuthorizationRules.length > 0) && providerAuthorization === false) {
        ...
        const templateParts = [vtlPrinter.print(compoundExpression(expressions)), resolver.Properties.ResponseMappingTemplate];
