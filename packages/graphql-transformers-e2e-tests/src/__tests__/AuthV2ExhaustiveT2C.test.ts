...
const STACK_NAME = `auth-exhaustive-tests-2c-${BUILD_TIMESTAMP}`;
const BUCKET_NAME = `auth-exhaustive-tests-bucket-2c-${BUILD_TIMESTAMP}`;
const AUTH_ROLE_NAME = `${STACK_NAME}-authRole`;
const UNAUTH_ROLE_NAME = `${STACK_NAME}-unauthRole`;
const LOCAL_FS_BUILD_DIR = `/tmp/auth_v2_exhaustive_tests_2c_${BUILD_TIMESTAMP}/`;
  ...
  authStrategies.forEach((strategy) => {
    ...
    providers.forEach((provider) => {
      AUTH_TEST_OPERATIONS.forEach((operation) => {
        const { modelName, schema } = generateTestModel(strategy, provider, operation, false, true);
        tests.push({
          modelName,
          strategy,
          provider,
