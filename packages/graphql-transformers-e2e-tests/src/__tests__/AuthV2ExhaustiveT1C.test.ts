...
const STACK_NAME = `auth-exhaustive-tests-1c-${BUILD_TIMESTAMP}`;
const BUCKET_NAME = `auth-exhaustive-tests-bucket-1c-${BUILD_TIMESTAMP}`;
const AUTH_ROLE_NAME = `${STACK_NAME}-authRole`;
const UNAUTH_ROLE_NAME = `${STACK_NAME}-unauthRole`;
const LOCAL_FS_BUILD_DIR = `/tmp/auth_v2_exhaustive_tests_1c_${BUILD_TIMESTAMP}/`;
