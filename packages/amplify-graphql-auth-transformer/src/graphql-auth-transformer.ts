...
export class AuthTransformer extends TransformerAuthBase implements TransformerAuthProvider {
  ...
  */
  ...
  addAutoGeneratedRelationalFields = (
    ...
    const inputTypeDefinitions = ctx.inputDocument.definitions.filter((it) => it.kind === 'ObjectTypeDefinition') as ObjectTypeDefinitionNode[];
    ...
    const typeDefinitions = [
      ...
      ...outputTypeDefinitions.filter(outputDef =>
        !inputTypeDefinitions.some(inputDef => inputDef.name.value === outputDef.name.value)
      )
    ];
