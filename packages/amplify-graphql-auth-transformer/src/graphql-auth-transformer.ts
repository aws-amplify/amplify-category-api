...
export class AuthTransformer extends TransformerAuthBase implements TransformerAuthProvider {
  ...
  */
   ...
   * @param ownerFields owner fields
   */
   ...
   * @param typeName type of the name
   * @param rules rules
   */
    ...
    fields.forEach((field) => {
      ...
      directives.forEach((directive) => {
        ...
        if (directive.name.value === 'hasOne'
          ...
          allowedFields.add(
            getConnectionAttributeName(ctx.featureFlags, def.name.value, field, getObjectPrimaryKey(relatedType).name.value),
          );
