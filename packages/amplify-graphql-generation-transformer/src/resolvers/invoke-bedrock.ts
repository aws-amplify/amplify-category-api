...
export const createInvokeBedrockResolverFunction = (
  ...
  const AI_MODEL = JSON.stringify(aiModel);
...
const generateResolver = (fileName: string, template: Record<string, string>) => {
  const resolver = fs.readFileSync(path.join(__dirname, fileName), 'utf8');
  const templated = resolver.replace(/({([^}]*)})/g, (_, k) => template[k]);
  return templated;
};
