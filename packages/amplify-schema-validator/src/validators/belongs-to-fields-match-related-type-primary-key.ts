...
export const validateBelongsToFieldsMatchRelatedTypePrimaryKey = (schema: DocumentNode): Error[] => {
    ...
    objectTypeDefinitions.forEach((objectTypeDefinition) => {
        ...
        belongsToDirectiveFields?.forEach((belongsToDirectiveField) => {
            const belongsTodirectiveArgs = belongsToDirectiveField.directives?.filter((belongsToDirective) => belongsToDirective.arguments && belongsToDirective.arguments.length > 0);
            belongsTodirectiveArgs?.forEach((belongsTodirectiveArg) => {
                ...
                primaryKeyDirectiveFields?.forEach((primaryKeyDirectiveField) => {
                    const primaryKeydirectiveArgs = primaryKeyDirectiveField.directives?.filter((primaryKeyDirective) => primaryKeyDirective.arguments && primaryKeyDirective.arguments.length > 0);
