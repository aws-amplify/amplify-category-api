...
export const validateManyToManyTwoLocations = (
  ...
  const allManyToManyDirectives = allObjectFields
    .map((objectField) => objectField.directives?.find(
  ...
  allManyToManyDirectives.forEach((directive) => {
    const relationNameArgument = directive?.arguments?.find(
      (argument) => argument.name.value === 'relationName',
    );
    const relationName = (relationNameArgument?.value as StringValueNode)
      ?.value;
    relationCounts[relationName] = relationCounts[relationName]
      ? relationCounts[relationName] + 1
      : 1;
  });

  const unmatchedRelations = Object.keys(relationCounts).filter(
    (relationName) => relationCounts[relationName] !== 2,
  );
  if (unmatchedRelations.length) {
    errors.push(new InvalidDirectiveError(
      `Invalid @manyToMany directive in schema: relation names '${unmatchedRelations.join(
        ', ',
      )}' must be used in exactly two locations.`,
    ));
  }
  return errors;
};
