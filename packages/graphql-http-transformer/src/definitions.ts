...
import {
  makeInputValueDefinition, makeNonNullType, makeNamedType, ModelResourceIDs, unwrapNonNull,
...
export function makeUrlParamInputObject(
  ...
  const urlParamFields = urlParams.map((param: string) => makeInputValueDefinition(param, makeNonNullType(makeNamedType('String'))));
  return {
