...
export class TransformerResolver implements TransformerResolverProvider {
  ...
  slotExists = (
    slotName: string,
    mappingTemplate?: FunctionRuntimeTemplate,
  ): boolean => this.findSlot(slotName, mappingTemplate) !== undefined;
  ...
  synthesize = (context: TransformerContextProvider, api: GraphQLAPIProvider): void => {
    ...
    [requestMappingTemplate, responseMappingTemplate, codeMappingTemplate].map((template) => this.substituteSlotInfo(template, 'main', 0));
