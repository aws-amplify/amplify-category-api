 ...
 *
 */
export const createSyncLambdaIAMPolicy = (stack: cdk.Stack, name: string, region?: string): iam.Policy => new iam.Policy(stack, 'InvokeLambdaFunction', {
  statements: [
    new iam.PolicyStatement({
      effect: iam.Effect.ALLOW,
...
const syncLambdaArnResource = (name: string, region?: string): string => {
  const substitutions = {};
