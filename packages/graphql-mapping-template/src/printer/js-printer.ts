...
class JsPrinter extends Printer {
  protected printIf(node: IfNode, indent: string = '') {
    if (node.inline) {
      return `if( ${this.printExpr(node.predicate, '')} ) { ${this.printExpr(node.expr, '')} }`;
    }
  ...
  protected printIfElse(node: IfElseNode, indent: string = '') {
    if (node.inline) {
      return `if( ${this.printExpr(node.predicate)} ) { ${this.printExpr(node.ifExpr)} } else { ${this.printExpr(node.elseExpr)} }`;
    }
    return (
  ...
  protected printForEach(node: ForEachNode, indent: string = ''): string {
    return (
      `${indent}for ( const ${this.printExpr(node.key)} in ${this.printExpr(node.collection)} ) {\n` +
  ...
  protected printSet(node: SetNode, indent: string = ''): string {
    return `${indent}${this.printReference(node.key)} = ${this.printExpr(node.value, '')}`;
  }
  ...
  protected printReturn(node: ReturnNode, indent: string = ''): string {
    ...
    return node.value ? `${indent}runtime.earlyReturn(${this.printExpr(node.value)})` : `${indent}runtime.earlyReturn()`;
  }
}

export const jsPrinter = new JsPrinter();
