...
export class SSMClient {
  ...
  // Support both v2 and v3 clients until provider is migrated
  ...
  deleteSecrets = async (secretNames: string[]): Promise<void> => {
    if (typeof (this.ssmClient as SSM_Client).send === 'function') {
      try {
        await (this.ssmClient as SSM_Client).send(new DeleteParametersCommand({ Names: secretNames }));
      } catch (err) {
        // if the value didn't exist in the first place, consider it deleted
        if (err?.name !== 'ParameterNotFound') {
          throw err;
      ...
      try {
        await (this.ssmClient as aws.SSM).deleteParameters({ Names: secretNames }).promise();
      } catch (err) {
        // if the value didn't exist in the first place, consider it deleted
        if (err?.code !== 'ParameterNotFound') {
          throw err;
...
const getSSMClient = async (context: $TSContext): Promise<aws.SSM | SSM_Client> => {
  ...
  // Return the client from provider (currently v2, will be v3 when provider is migrated)
  return client;
};
