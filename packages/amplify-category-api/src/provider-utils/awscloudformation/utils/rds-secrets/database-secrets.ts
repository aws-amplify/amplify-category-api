...
import { getParameterStoreSecretPath, RDSConnectionSecrets, ImportedRDSType } from '@aws-amplify/graphql-transformer-core';
import {
  MySQLDataSourceAdapter, Schema, Engine, DataSourceAdapter, MySQLDataSourceConfig,
...
export const deleteConnectionSecrets = async (context: $TSContext, secretsKey: string, apiName: string, envName?: string) => {
  ...
  const ssmClient = await SSMClient.getInstance(context);
  const secretParameterPaths = secretNames.map((secret) => getParameterStoreSecretPath(secret, secretsKey, apiName, environmentName, appId));
