...
async function getRdsClusterResourceIdFromArn(arn: string | undefined, AWSConfig) {
  ...
  const describeDBClustersResult = await RDS.send(new DescribeDBClustersCommand({}));
...
async function getSecretStoreArn(context: $TSContext, inputs, clusterResourceId, AWSConfig) {
  ...
  const listSecretsResult = await SecretsManager.send(new ListSecretsCommand(params));

  rawSecrets = listSecretsResult.SecretList;
  let token = listSecretsResult.NextToken;
  while (token) {
    params[NextToken] = token;
    const tempSecretsResult = await SecretsManager.send(new ListSecretsCommand(params));
...
async function selectDatabase(context: $TSContext, inputs, clusterArn, secretArn, AWSConfig) {
  ...
  try {
    const dataApiResult = await DataApi.send(new ExecuteStatementCommand(params));
