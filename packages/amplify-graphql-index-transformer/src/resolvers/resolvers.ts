...
function setSyncQueryFilterSnippet(deltaSyncTableTtl: number) {
  ...
  expressions.push(
    compoundExpression([
      ...
      generateDeltaTableTTLCheck("isLastSyncInDeltaTTLWindow", deltaSyncTableTtl, "ctx.args.lastSync"),
      ifElse(
...
const generateDeltaTableTTLCheck = (deltaTTLCheckRefName: string, deltaTTLInMinutes: number, lastSyncRefName: string): Expression => {
...
export const getDeltaSyncTableTtl = (resourceOverrides: any, resource: TransformerResolverProvider) => {
  ...
  const deltaSyncTtlOverride = _.get(resourceOverrides, [
    "models",
    modelName,
    "modelDatasource",
    "dynamoDbConfig",
